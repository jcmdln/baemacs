...
When native compilation lands in upstream Emacs, this section will be removed.

This is optional but highly recommended for those seeking improved
performance. The following example shows how I compile my own equivalent of
~emacs-nox~ for Fedora though it should be trivial to adapt it to your target
platform.

#+BEGIN_SRC shell :eval no :tangle no
  $ sudo dnf install -y automake gnutls-devel gpm-devel jansson-devel \
    libgccjit-devel ncurses-devel systemd-devel
  $ git clone --branch feature/native-comp --depth 1 \
    https://github.com/emacs-mirror/emacs
  $ cd emacs
  $ ./autogen.sh
  $ ./configure \
    CC="gcc" \
    CFLAGS="-O3 -g3 -march=native -pipe" \
    --without-all \
    --without-x \
    --with-dbus \
