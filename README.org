...
equivalent of ~emacs-nox~ for Fedora 34+ though it should be trivial to adapt
it to your target platform.
...
check for, each file listed in this section will detail their contents.
...
**** async
Adds the ability to call asynchronous functions and process with ease. See the
documentation for ~async-start~ and ~async-start-process~.

https://stable.melpa.org/#/async
...
https://stable.melpa.org/#/auto-compile
