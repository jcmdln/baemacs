# -*- mode : org -*-
#+TITLE: Baemacs - A Literate GNU/Emacs Configuration in Org-Mode
#+STARTUP: indent
#+LANGUAGE: en
...
- [[#configuration-files][Configuration Files]]
  - [[#initel][init.el]]
    ...
    - [[#handling-custom-configuration][Handling Custom Configuration]]
    - [[#encoding][Encoding]]
    - [[#environment][Environment]]
    - [[#appearance][Appearance]]
      - [[#disable-toolbars][Disable toolbars]]
      - [[#disable-startup-buffers-and-actions][Disable startup buffers and actions]]
      - [[#re-use-frames-rather-than-creating-new-ones][Re-use frames rather than creating new ones]]
      - [[#set-a-basic-theme-and-font][Set a basic theme and font]]
      - [[#improve-editing-generic-files][Improve editing generic files]]
      - [[#show-line-numbers-when-editing-files][Show line numbers when editing files.]]
...
collection is deferred until reaching 67% of the total heap size.  After
init completes, we lower this value to 34% of the total heap size.

#+BEGIN_SRC emacs-lisp :tangle init.el
  (setq gc-cons-percentage-default gc-cons-percentage
        gc-cons-percentage-init    0.667
        gc-cons-percentage-tuned   0.334
...
Now we will increase the `gc-cons-threshold`, which refers to the
...
is 800KB.  The idea is that by specifying a larger threshold, garbage
collection will occur less often, meaning fewer GC pauses.

We'll set this value to 2MB globally, which will significantly reduce
the total number of GC pauses we experience, also storing the default
value in case it must be referenced.
...
always precedes EOF, tweak the paren matching delay, and disable the
visual bell:
